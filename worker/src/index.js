...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    // the desk's Settings panel is where the owner copies it into n8n.
    if (apiPath === '/n8n/queue' && effective.method === 'GET') {
    ...
    if (apiPath === '/n8n/queue/push' && effective.method === 'POST') {
      ...
      if (!venture || !platform || !category || !text) {
        return apiJson({ ok: false, error: 'venture, platform, category and text are all required.' }, 400);
      }
      if (!(await getVenture(db, venture))) return apiJson({ ok: false, error: `Unknown venture "${venture}".` }, 400);
      if (!isPlatform(platform)) return apiJson({ ok: false, error: `Unknown platform "${platform}".` }, 400);
      if (!isCategory(category)) return apiJson({ ok: false, error: `Unknown category "${category}".` }, 400);
      const id = await insertPost(db, {
        venture, platform, category, text,
        image_url: body.image_url || null,
        link: body.link || null,
        source_article: body.source_article || null,
        status: 'queued',
        scheduled_for: body.scheduled_for || null,
        notes: body.notes || 'Pushed from n8n.'
      });
      return apiJson({ ok: true, id }, 201);
    }

    if (apiPath === '/n8n/generate' && effective.method === 'POST') {
      const provided = (effective.headers.get('Authorization') || '').replace(/^Bearer\s+/i, '');
      if (!(await verifyN8nToken(env, provided))) return apiJson({ ok: false, error: 'Unauthorized' }, 401);
      if (!hasStore(env)) return apiJson({ ok: false, error: 'D1 is not connected. See worker/README.md.' }, 503);
      if (!env.ANTHROPIC_API_KEY) return apiJson({ ok: false, error: 'ANTHROPIC_API_KEY is not set on the Worker' }, 503);
      ...
      // Tell n8n what was written, if an address is set. Awaited rather than
      // fired and forgotten so the answer can say whether it actually landed:
      // a caller that is told "created 6" and never hears that the forward
      // failed has no way to know the other half did not happen. It cannot fail
      // the request, only report itself.
      const forwarded = await forwardToN8n(env, 'social.generated', {
    ...
    if (apiPath === '/author-desk/media-pack' && effective.method === 'POST') {
      // Two ways in, because one of them was unreachable in practice.
      // AUTHOR_DESK_TRIGGER_TOKEN is a Cloudflare dashboard secret, and the
      // owner has no dashboard, so that door was permanently shut. The n8n
      // token is the same class of credential, generated by the Worker and
      // readable from the desk, so it opens this too. The secret still works
      // where one is set, and with neither this stays closed rather than
      // becoming an open endpoint that anyone who finds the URL can use to
      // force a rebuild and spend Anthropic tokens.
      const provided = (effective.headers.get('Authorization') || '').replace(/^Bearer\s+/i, '');
      const expected = env.AUTHOR_DESK_TRIGGER_TOKEN;
      const bySecret = Boolean(expected) && provided === expected;
      const byN8nToken = await verifyN8nToken(env, provided);
      if (!bySecret && !byN8nToken) {
