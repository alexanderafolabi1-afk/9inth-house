...
Both are required. The Worker checks for both at the start of every run and
logs a plain error (never the secret value itself) and stops if either is
missing.
...
Monday or Thursday, so every desk fires) comes to roughly 26, comfortably
under the limit.
...
API key or GitHub token anywhere (that is exactly what "never print secrets"
...
3. Run `wrangler secret put ANTHROPIC_API_KEY` and
   `wrangler secret put GITHUB_TOKEN`, pasting each value when prompted.
